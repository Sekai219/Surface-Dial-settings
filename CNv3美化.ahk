...
Menu, Tray, Add, Dial Settings-Sekai , AuthorInfo
Menu, Tray, Add, 使用说明, ShowInstructions
...
; 使用说明窗口显示
ShowInstructions:
Gui, New, +AlwaysOnTop +Resize +MinimizeBox +MaximizeBox +ToolWindow +Border
Gui, Font, s12, Segoe UI
Gui, Color, EFEFEF
Gui, Margin, 20, 20
Gui, Add, Text,, **注意事项**
Gui, Add, Text,, 区域1：与系统设置界面的自定义键相同，路径为（windows系统设置-设备-轮盘-加号-自定义）。
Gui, Add, Text,, 区域2：先按Caps键切换到大写模式，再进行输入。
Gui, Add, Text,, 请不要设置方向键（与默认配置冲突），方向与滚动建议直接使用默认模式。
Gui, Add, Text,, **配置文件**
Gui, Add, Text,, 1.保存配置将生成配置文件，请确保配置文件default.ini/Custom.ini和软件在同一目录或者文件夹下。
Gui, Add, Text,, 2.可以通过设置界面重置默认设置还原配置文件。
Gui, Add, Text,, 3.重置将删除已有自定义文件，如果需要可以备份Custom.ini文件。
Gui, Add, Text,, 点击下方蓝色链接-反馈Bug-提出建议-获取帮助
Gui, Add, Text, cBlue gOpenGitHub, GitHub: https://github.com/Sekai219
Gui, Add, Button, w1 h1 Default, 
Gui, Show,, 使用说明
...
AuthorInfo:
Gui, New, +AlwaysOnTop +MinimizeBox +ToolWindow +Border
Gui, Font, s12, Segoe UI
Gui, Color, EFEFEF
Gui, Margin, 20, 20
Gui, Add, Text,,  如果有所帮助，GitHub加星标，关注持续更新！
Gui, Add, Text,, - - - - - - - - - - - - - - - - - - - - - - - - - - Sekai
Gui, Add, Text, cBlue gOpenGitHub, GitHub: https://github.com/Sekai219
Gui, Add, Button, w1 h1, 
Gui, Show,, 关于作者
return
